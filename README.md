# ronin-snapshot
Compressed database, block number = `0xabb266`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0xabb266.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0xabb266.md5
md5sum -c checksum-0xabb266.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0xabb266.tar
 ```
