# ronin-snapshot
Compressed database, block number = `0xd25ded`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0xd25ded.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0xd25ded.md5
md5sum -c checksum-0xd25ded.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0xd25ded.tar
 ```
