# ronin-snapshot
Compressed database, block number = `0xe615ed`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0xe615ed.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0xe615ed.md5
md5sum -c checksum-0xe615ed.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0xe615ed.tar
 ```
