# ronin-snapshot
Compressed database, block number = `0x11ef4e2`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0x11ef4e2.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0x11ef4e2.md5
md5sum -c checksum-0x11ef4e2.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0x11ef4e2.tar
 ```
