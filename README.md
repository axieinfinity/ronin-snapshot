# ronin-snapshot
Compressed database, block number = `0xdb986c`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0xdb986c.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0xdb986c.md5
md5sum -c checksum-0xdb986c.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0xdb986c.tar
 ```
