# ronin-snapshot
Compressed database, block number = `0x8ca9be`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0x8ca9be.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0x8ca9be.md5
md5sum -c checksum-0x8ca9be.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0x8ca9be.tar
 ```
