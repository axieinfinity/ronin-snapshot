# ronin-snapshot
Compressed database, block number = `0xbb13d4`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0xbb13d4.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0xbb13d4.md5
md5sum -c checksum-0xbb13d4.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0xbb13d4.tar
 ```
