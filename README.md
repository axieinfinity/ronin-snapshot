# ronin-snapshot
Compressed database, block number = `0x1194f7e`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0x1194f7e.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0x1194f7e.md5
md5sum -c checksum-0x1194f7e.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0x1194f7e.tar
 ```
