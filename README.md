# ronin-snapshot
Compressed database, block number = `0x86f68c`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0x86f68c.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0x86f68c.md5
md5sum -c checksum-0x86f68c.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0x86f68c.tar
 ```
