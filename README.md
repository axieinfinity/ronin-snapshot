# ronin-snapshot
Compressed database, block number = `0x19b4e5f`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0x19b4e5f.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0x19b4e5f.md5
md5sum -c checksum-0x19b4e5f.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0x19b4e5f.tar
 ```
