# ronin-snapshot
Compressed database, block number = `0xbdb675`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0xbdb675.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0xbdb675.md5
md5sum -c checksum-0xbdb675.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0xbdb675.tar
 ```
