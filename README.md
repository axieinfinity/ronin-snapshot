# ronin-snapshot
Compressed database, block number = `0xcbc66f`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0xcbc66f.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0xcbc66f.md5
md5sum -c checksum-0xcbc66f.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0xcbc66f.tar
 ```
