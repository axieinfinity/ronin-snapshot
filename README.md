# ronin-snapshot
Compressed database, block number = `0xcfbaed`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0xcfbaed.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0xcfbaed.md5
md5sum -c checksum-0xcfbaed.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0xcfbaed.tar
 ```
