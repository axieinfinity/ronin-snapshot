# ronin-snapshot
Compressed database, block number = `0xdeabec`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0xdeabec.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0xdeabec.md5
md5sum -c checksum-0xdeabec.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0xdeabec.tar
 ```
