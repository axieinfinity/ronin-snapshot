# ronin-snapshot
Compressed database, block number = `0xdab76c`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0xdab76c.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0xdab76c.md5
md5sum -c checksum-0xdab76c.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0xdab76c.tar
 ```
