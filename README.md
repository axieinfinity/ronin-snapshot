# ronin-snapshot
Compressed database, block number = `0x902d9a`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0x902d9a.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0x902d9a.md5
md5sum -c checksum-0x902d9a.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0x902d9a.tar
 ```
