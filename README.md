# ronin-snapshot
Compressed database, block number = `0xfccbff`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0xfccbff.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0xfccbff.md5
md5sum -c checksum-0xfccbff.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0xfccbff.tar
 ```
