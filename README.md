# ronin-snapshot
Compressed database, block number = `0xa2e8ac`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0xa2e8ac.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0xa2e8ac.md5
md5sum -c checksum-0xa2e8ac.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0xa2e8ac.tar
 ```
