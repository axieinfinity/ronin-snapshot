# ronin-snapshot
Compressed database, block number = `0x12dc9ea`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0x12dc9ea.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0x12dc9ea.md5
md5sum -c checksum-0x12dc9ea.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0x12dc9ea.tar
 ```
