# ronin-snapshot
Compressed database, block number = `0xd09bed`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0xd09bed.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0xd09bed.md5
md5sum -c checksum-0xd09bed.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0xd09bed.tar
 ```
