# ronin-snapshot
Compressed database, block number = `0xa97fee`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0xa97fee.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0xa97fee.md5
md5sum -c checksum-0xa97fee.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0xa97fee.tar
 ```
