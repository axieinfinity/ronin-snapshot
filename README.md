# ronin-snapshot
Compressed database, block number = `0xba32d8`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0xba32d8.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0xba32d8.md5
md5sum -c checksum-0xba32d8.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0xba32d8.tar
 ```
