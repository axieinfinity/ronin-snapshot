# ronin-snapshot
Compressed database, block number = `0xd965ed`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0xd965ed.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0xd965ed.md5
md5sum -c checksum-0xd965ed.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0xd965ed.tar
 ```
