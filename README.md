# ronin-snapshot
Compressed database, block number = `0x1ab1e2f`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0x1ab1e2f.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0x1ab1e2f.md5
md5sum -c checksum-0x1ab1e2f.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0x1ab1e2f.tar
 ```
