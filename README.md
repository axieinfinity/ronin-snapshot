# ronin-snapshot
Compressed database, block number = `0xb32add`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0xb32add.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0xb32add.md5
md5sum -c checksum-0xb32add.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0xb32add.tar
 ```
