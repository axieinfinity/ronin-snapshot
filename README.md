# ronin-snapshot
Compressed database, block number = `0x19bbee0`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0x19bbee0.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0x19bbee0.md5
md5sum -c checksum-0x19bbee0.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0x19bbee0.tar
 ```
