# ronin-snapshot
Compressed database, block number = `0x15abfbd`.
...
```shell
curl -O -L -k https://storage.googleapis.com/chaindata/chaindata-0x15abfbd.tar
curl -O -L -k https://storage.googleapis.com/chaindata/checksum-0x15abfbd.md5
md5sum -c checksum-0x15abfbd.md5
```
2. Uncompress downloaded files:
```shell
tar -xvf chaindata-0x15abfbd.tar
 ```
